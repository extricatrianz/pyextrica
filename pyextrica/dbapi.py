...
import pyextrica.client
import pyextrica.exceptions
import pyextrica.logging
from pyextrica import constants
from pyextrica.constants import LENGTH_TYPES, PRECISION_TYPES, SCALE_TYPES
...
from pyextrica.transaction import NO_TRANSACTION, IsolationLevel, Transaction
...
class Connection(object):
    ...
    def __init__(
        ...
        self._client_session = pyextrica.client.ClientSession(
            user=user,
            catalog=catalog,
            schema=schema,
            source=source,
        ...
        if http_session is None:
            self._http_session = pyextrica.client.TrinoRequest.http.Session()
    ...
    def _create_request(self):
        return pyextrica.client.TrinoRequest(
            self.host,
            self.port,
...
class Cursor(object):
    ...
    def setinputsizes(self, sizes):
        raise pyextrica.exceptions.NotSupportedError
    ...
    def _prepare_statement(self, statement: str, name: str) -> None:
        ...
        query = pyextrica.client.TrinoQuery(self.connection._create_request(), query=sql,
    ...
    def _execute_prepared_statement(
        ...
        return pyextrica.client.TrinoQuery(self._request, query=sql, legacy_primitive_types=self._legacy_primitive_types)
    ...
    def _execute_immediate_statement(self, statement: str, params):
        ...
        return pyextrica.client.TrinoQuery(
    ...
    def _format_prepared_param(self, param):
        ...
        raise pyextrica.exceptions.NotSupportedError("Query parameter of type '%s' is not supported." % type(param))
    ...
    def execute(self, operation, params=None):
        if params:
            ...
            self._query = pyextrica.client.TrinoQuery(self._request, query=operation,
                                                  legacy_primitive_types=self._legacy_primitive_types)
            self._iterator = iter(self._query.execute())
        return self
    ...
    def fetchone(self) -> Optional[List[Any]]:
        ...
        try:
            ...
            return None
        except pyextrica.exceptions.HttpError as err:
            raise pyextrica.exceptions.OperationalError(str(err))
    ...
    def describe(self, sql: str) -> List[DescribeOutput]:
        ...
        try:
            ...
            self._query = pyextrica.client.TrinoQuery(
                self._request,
                query=sql,
                legacy_primitive_types=self._legacy_primitive_types,
            )

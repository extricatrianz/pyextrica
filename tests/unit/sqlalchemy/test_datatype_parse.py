...
from pyextrica.sqlalchemy.datatype import MAP, ROW, TIME, TIMESTAMP
...
from pyextrica.sqlalchemy.datatype import _FormatTypeMixin

# Create a mock class that inherits from _FormatTypeMixin for testing purposes
class FormatTypeMixinTest(_FormatTypeMixin):
    def _format_value(self, value):
        return str(value)  # Mock implementation for testing

class TestFormatTypeMixin(unittest.TestCase):
    def test_bind_processor(self):
        mixin = FormatTypeMixinTest()

        # Mock the string_bind_processor method
        mixin.string_bind_processor = Mock(return_value=lambda x: x.upper())

        processor = mixin.bind_processor(Mock())
        result = processor("hello")

        self.assertEqual(result, "HELLO")  # Check if the processor works as expected

    def test_literal_processor(self):
        mixin = FormatTypeMixinTest()

        # Mock the string_literal_processor method
        mixin.string_literal_processor = Mock(return_value=lambda x: f"'{x}'")

        processor = mixin.literal_processor(Mock())
        result = processor("hello")
...
from pyextrica.sqlalchemy.datatype import _JSONFormatter, JSONIndexType, JSONPathType
